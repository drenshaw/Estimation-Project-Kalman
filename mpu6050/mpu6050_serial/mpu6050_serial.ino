...
int read_gyro_accel_vals(uint8_t* accel_t_gyro_ptr) {
  ...
  //int error = MPU6050_read (EXT_SENS_DATA_00, (uint8_t *) accel_t_gyro, sizeof(*accel_t_gyro));
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  uint32_t tK1 = micros();
  // Apply Kalman Filter to both x,y axes (z doesn't get filtered)
  float kal_x = kalmanFilterX(accel_angle_x, gyro_x, dt);
  float kal_y = kalmanFilterY(accel_angle_y, gyro_y, dt);
  float kal_z = gyro_angle_z;  //Accelerometer doesn't give z-angle
  uint32_t tK2 = micros();
  uint32_t tK = tK2 - tK1;
...
// Force to keep values within -180 to 180 degrees
  // X accel
  if (accel_angle_x < -180)
  {
    accel_angle_x += 360;
  }
  else if (accel_angle_x > 180)
  {
    accel_angle_x -= 360;
  }
  // Y accel
  if (accel_angle_y < -180)
  {
    accel_angle_y += 360;
  }
  else if (accel_angle_y > 180)
  {
    accel_angle_y -= 360;
  }
  // Z accel
  if (accel_angle_z < -180)
  {
    accel_angle_z += 360;
  }
  else if (accel_angle_z > 180)
  {
    accel_angle_z -= 360;
  }
 // GYRO
  ...
  else if (gyro_angle_z > 180)
  {
    gyro_angle_z -= 360;
  }

  uint32_t del_t = tK/tC;
  ...
  Serial.print(F("#MAG:"));             //Magnetometer angle
  ...
  Serial.print(F("#KAL:"));             //Filtered angle
  Serial.print(kal_x, 2);
  Serial.print(F(","));
  Serial.print(kal_y, 2);
  Serial.print(F(","));
  Serial.print(kal_z, 2);
//  Serial.print("TIM:");
//  Serial.print(tC);
//  Serial.print(F(","));
//  Serial.print(tK);
